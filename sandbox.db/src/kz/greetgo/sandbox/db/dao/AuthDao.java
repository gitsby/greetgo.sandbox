...
public interface AuthDao {
  ...
  @Select("select client from Person where accountName = #{accountName} " +
  ...
  @Select("select accountName from Person where client = #{personId}")
  String accountNameByPersonId(@Param("personId") String personId);

  @Select("select * from Person where client = #{personId}")
